...
@InternalApi private[typed] class ActorContextAdapter[T](val untyped: a.ActorContext) extends ActorContext[T] {
  ...
  override def watch[U](other: ActorRef[U]) = { untyped.watch(toUntyped(other)); other }
  override def unwatch[U](other: ActorRef[U]) = { untyped.unwatch(toUntyped(other)); other }
